...
export function BlogHeader({ locale, switchPaths }: { locale: BlogLocale; switchPaths: Record<BlogLocale, string> }) {
  ...
  return (
    <header className="border-b border-mist bg-white">
      <div className="mx-auto flex max-w-3xl items-center justify-between gap-2 px-4 py-2 sm:px-5">
        ...
        <nav className="flex items-center gap-3 text-sm sm:gap-4">
          <Link href={blogIndexPath(locale)} className="inline-flex min-h-11 items-center font-medium text-ink-900 hover:text-green-700">
            {ui.breadcrumbBlog}
          </Link>
          <div className="flex items-center gap-1.5 text-ink-500 sm:gap-2" aria-label={ui.otherLanguages}>
            {(Object.keys(switchPaths) as BlogLocale[])
              .filter((l) => l !== locale)
              .map((l) => (
                <Link key={l} href={switchPaths[l]} hrefLang={l} className="inline-flex min-h-11 items-center hover:text-ink-900">
                  {LOCALE_LABEL[l]}
                </Link>
              ))}
          </div>
...
export function BlogFooter({ locale }: { locale: BlogLocale }) {
  ...
  return (
    <footer className="border-t border-mist bg-white">
      <div className="mx-auto flex max-w-3xl items-center justify-between gap-2 px-4 py-3 text-sm text-ink-500 sm:px-5">
