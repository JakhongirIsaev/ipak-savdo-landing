...
- **Shadows:** Минимум. `shadow-[0_1px_2px_rgba(11,24,38,0.04)]` на card surfaces (HeroStatCard, product mock cards). `shadow-[0_8px_32px_rgba(11,24,38,0.08)]` на Cookie banner. Никаких `shadow-2xl`, никаких glow.
  - **Device-elevation (исключение для product mockups):** телефонный фрейм в Hero/ForOwner — `shadow-[0_28px_70px_-30px_rgba(11,24,38,0.35)]`; плавающий «today» chip в Hero — `shadow-[0_18px_40px_-18px_rgba(11,24,38,0.28)]`. Отрицательный spread держит тень собранной и заземлённой (device elevation, не glow). Применять **только** к device/floating product-моментам, не к плоским card-поверхностям.
...
Pill, не rounded-md. Hover уходит в `green-800`. Иконка справа (`ArrowRight size={16} strokeWidth={1.75}`).
...
- **Color contrast:** ink-900 на paper, ink-700 на paper, ink-500 на paper — все WCAG AA для тела (≥4.5:1). Заливные CTA используют green-700 (#027F2E) с белым текстом для контраста AA; green-500 — для индикаторов/акцентов, не для текста на нём.
- **Semantic HTML:** Section/header/nav/footer используются. h1 единственный (hero title), h2/h3 по иерархии.

## Internationalization

- **Locales:** `uz` (default, маршрут `/`) + `ru` (маршрут `/ru`). Локаль задаётся маршрутом (`initialLocale` в `app/page.tsx` / `app/ru/page.tsx`), переключатель — обычные ссылки между `/` и `/ru`.
- **Both dicts MUST stay structurally identical** (`typeof ru === typeof uz`) — правило для `copy` в `components/concept/ConceptLanding.tsx` и словарей в `lib/landing/i18n.ts` (живой только `faq`-сабсет).
...
**Действие:** Удалить эти файлы. Канонический live-landing — `components/concept/ConceptLanding.tsx` + `components/concept/*` (рендерится из `app/page.tsx` = uz и `app/ru/page.tsx` = ru); `components/LandingPage.tsx` и `components/landing/*` тоже legacy/dead, не редактируйте их в расчёте на live. Также: `.swarm/*` директории содержат старые worktree копии — удалить целиком.
...
| 2026-05-29 | CTA fill darkened green-500→green-700 for WCAG | green-500 reserved for live-dot/accents. |
| 2026-05-30 | Product-forward redesign: in-page CSS mockups replace screenshots | Hero/ForOwner/HowItWorks теперь рендерят BirLiy-интерфейс в коде (PhoneFrame + PosSaleScreen / OwnerDashboard / 6 StepScreen в `components/landing/mockups.tsx`) вместо `public/product/*.png`. Убирает протёкшие тест-данные («APK Smoke», «UZS»), даёт «настоящий продукт» на лендинге, остаётся на бренд-токенах. |
| 2026-05-30 | Device-elevation shadows added | Телефонные фреймы и floating chip требуют объёма; задокументированы выше (не glow). |
| 2026-05-30 | StepScreen chrome — ink, не green symbol | Зелёный бренд-символ ×6 в сетке шагов нарушал Green Rule; chrome переведён в ink-моно, зелёный оставлен только на смысловом сигнале (QR-чип, success-check) + CTA. |
| 2026-05-30 | Telegram-канал `@bir_liy` добавлен на сайт | Header (иконка + mobile-меню), Footer (Контакт), отдельная секция `TelegramBand` перед lead-формой, ссылка в Hero. Монохромная (ink) иконка — не бренд-синий Telegram, ради палитры. |
| 2026-05-30 | Mobile-меню (hamburger) в Header | На <md теперь есть навигация + Telegram + CTA в выпадающей панели; раньше мобильный header был только лого + язык. |
| 2026-06-10 | Канонический лендинг = `components/concept/ConceptLanding.tsx` | Живая страница `/` и `/ru` рендерит concept-дерево; `components/LandingPage.tsx` и `components/landing/*` (кроме `JsonLd.tsx`) — мёртвое наследие, не редактировать в ожидании живых изменений. Локали маршрутные (`/` = uz, `/ru` = ru). Все ссылки на LandingPage.tsx выше в этом файле — исторические. |
